...
public class NewConnectionThread extends Thread
{
		...
		while(active)
		{
			try
			{
				...
				else if(inbox[0].equals("/CREATE NEW USER"))
				{
					...
					String username = (String)inbox[1];
					String password = (String)inbox[2];
					String email = (String)inbox[3];
					String firstName = (String)inbox[4];
					String lastName = (String)inbox[5];
					String birthDate = (String)inbox[6];
					
					  if(DataBase.availabilityCheck(username, email) == true)
					    {
						  
					    	DataBase.createNewUser(username, password, email, firstName, lastName, birthDate);
					        ServerSide.createUserFolders(username);
				...
				else if(inbox[0].equals("/LOG IN"))
				{
					...
					System.err.println("Received password (NewConnectionThread) : "+password);
				    ...
				    if(DataBase.loginCheck(username, password))
				    {
				    	...
				    	Object[] userIdentityInfo = DataBase.getUserFullNameAndID(username, password);
				    	...
				    	Object[] userInfo = DataBase.getUserInfo(username, password);
	...
	public void sendDirectMessage(final String message,final String sender_fullName,final String time,int sent_from,int sent_to)
	{
